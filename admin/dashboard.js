...
(() => {
  ...
  function renderOrderDetail() {
    const o = orders.find(x => x.id === openOrderDetailId);
    if (!o) return;
  ...
  function buildReceiptHtml(order) {
    ...
    return `
      <div class="receipt">
        ...
        ${order.order_type === 'takeout' && !order.is_pickup && order.customer_address ? `<p class="receipt-note">📍 ${escapeHtml(order.customer_address)}</p>` : ''}
  ...
  const invSortBy = document.getElementById('invSortBy');
  ...
  function renderInventoryList() {
    ...
    if (invSortBy.value === 'value_desc') {
      filtered.sort((a, b) => (Number(b.quantity) * Number(b.cost_per_unit) || 0) - (Number(a.quantity) * Number(a.cost_per_unit) || 0));
    }
  ...
  function sendNewOrderWhatsApp(order) {
    ...
    const header = order.order_type === 'takeout'
      ? `${order.is_pickup ? '🚶 წამოსვლა' : '🥡 გასატანი'}${order.customer_name ? ` — ${order.customer_name}` : ''}`
